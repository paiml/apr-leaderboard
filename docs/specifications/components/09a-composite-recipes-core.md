...
|-----------|-------|-------|
| NF4 base weights | ~4.0 GB | 8B params x 4 bits |
| LoRA A matrices (28 layers x Q,V) | ~6.1 MB | 56 x rank x hidden_dim x 2 bytes |
| LoRA B matrices (28 layers x Q,V) | ~6.1 MB | 56 x hidden_dim x rank x 2 bytes |
| Optimizer states (AdamW) | ~24.4 MB | 2 x LoRA params x 4 bytes (m, v) |
...
|-------|-----------|--------|-------|
| `lora_forward` | d_in=3584, rank=16 | 54us actual (CPU) | Real matmul, not analytical |
| `optimizer` | 6.4M LoRA params | 50us analytical | SIMD AdamW over LoRA params |
| `loss` | vocab=152064, seq=128 | 20us analytical | Cross-entropy |
| `train_step` | 28 layers, rank-16 | 5000us analytical | Composite fwd+bwd+optim |
