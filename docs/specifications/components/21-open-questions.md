...
26. ✅ **CPU throughput floor:** 2.5 tok/s on CPU for 1.5B Q4K — is this acceptable for batch eval, or should eval always target GPU? *Answered: CPU eval works but is slow. HumanEval (164 problems): ~4h. MBPP (974 problems): ~48h estimated. Two concurrent evals saturate CPU at ~450% total. CPU is viable for verification; GPU (wgpu) required for production eval at scale.*
27. ✅ **SCoT on small models:** Does structured chain-of-thought prompting improve code quality on 1.5B models? *Answered: SCoT correctly produces reasoning before code, but on 1.5B with 512-token budget, reasoning consumes all tokens — no code is generated. SCoT needs ≥1024 tokens or ≥7B models to be effective (§24.3).*
28. ✅ **HF parity via compare-hf:** `apr compare-hf` returns 0 comparisons on GGUF Q4K imports (dtype mismatch with HF FP16). *Answered: Expected behavior — Q4K uses different dtypes (Q4_K/Q6_K) than HF FP16/BF16, so element-wise comparison isn't meaningful. Parity must be verified indirectly via benchmark scores. 1.5B-Instruct Q4K tracks at ~70% HumanEval vs 67.6% HF reference (§24.4).*
