...
16. ✅ **GPU inference readiness:** GPU compute via wgpu (Vulkan/Metal/DX12) or optional CUDA. *GPU is mandatory for production eval — never fall back to CPU. On Blackwell sm_121, use `SKIP_PARITY_GATE=1` to bypass FP rounding parity check (five-whys: §22.19.2). GPU QLoRA training works correctly (wgpu NF4 blocks, LoRA backward GEMM verified).*
