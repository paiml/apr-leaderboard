...
27. ✅ **SCoT on small models:** Does structured chain-of-thought prompting improve code quality on 1.5B models? *Answered: SCoT correctly produces reasoning before code, but on 1.5B with 512-token budget, reasoning consumes all tokens — no code is generated. SCoT needs ≥1024 tokens or ≥7B models to be effective (§22.18).*
28. **HF parity via compare-hf:** `apr compare-hf` returns 0 comparisons on GGUF Q4K imports (dtype mismatch with HF FP16). Is this expected, or should compare-hf dequantize for comparison?
