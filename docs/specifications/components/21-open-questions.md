...
16. **GPU inference readiness:** GPU compute via wgpu (Vulkan/Metal/DX12) or optional CUDA. GPU is mandatory — never fall back to CPU. **Status (GH-559): BROKEN on Blackwell sm_121.** Q4K dequantization has systematic 5-20% per-super-block error. Parity gate correctly blocks GPU (cosine=-0.005). `SKIP_PARITY_GATE=1` is **forbidden** (Toyota Way). Fix needed in trueno-gpu Q4K PTX codegen. GPU QLoRA training works correctly (wgpu NF4 blocks, LoRA backward GEMM verified).
